...
pub struct ResultValue {
    ...
    pub attributes: Vec<AnyAttribute<'static>>,
    /// The captured the inner element
    pub inner: AnyElement<'static>,
}
...
impl<'xml> FromXml<'xml> for ResultValue {
    ...
    fn deserialize<'cx>(
        ...
        loop {
            match deserializer.next() {
                Some(Ok(Node::Attribute(attr))) => {
                    ...
                    attributes.push(AnyAttribute {
                        ns: id.ns.to_owned().into(),
                        name: id.name.to_owned().into(),
                        value: attr.value.into_owned().into(),
                    });
                }
                Some(Ok(Node::Open(element))) => {
                    let mut nested = deserializer.nested(element);
                    <AnyElement as FromXml>::deserialize(
                        &mut inner,
                        "ResultValue::inner",
                        &mut nested,
                    )?;
        ...
        *into = Some(Self {
            attributes,
            inner: Accumulate::<AnyElement>::try_done(inner, "ResultValue::inner")?.into_owned(),
        });
        Ok(())
    }
...
mod tests {
    ...
    fn error_value_attrs() {
        ...
        assert_eq!(
            ext_value.value.attributes[0].ns,
            "urn:ietf:params:xml:ns:suggestion-1.0"
        );
        assert_eq!(ext_value.value.attributes[0].name, "suggestedValue");
        assert_eq!(ext_value.value.attributes[0].value, "pqr-123");
    ...
    fn poll_unhandled_namespace() {
        ...
        assert_eq!(status.attributes[0].name, "s");
        assert_eq!(status.attributes[0].value, "ok");
        // domain:contact with type attribute
        let contact = &ev0.value.inner.children[4];
        assert_eq!(contact.name, "contact");
        assert_eq!(contact.attributes[0].name, "type");
        assert_eq!(contact.attributes[0].value, "admin");
        ...
        assert_eq!(ev1.value.inner.attributes[0].name, "state");
        assert_eq!(ev1.value.inner.attributes[0].value, "after");
        ...
        assert_eq!(case_id.attributes[0].name, "type");
        assert_eq!(case_id.attributes[0].value, "urs");
    ...
    fn domain_info_unhandled_namespace() {
        ...
        assert_eq!(rgp_status.attributes[0].name, "s");
        assert_eq!(rgp_status.attributes[0].value, "redemptionPeriod");
        assert_eq!(
            ev.reason.text,
